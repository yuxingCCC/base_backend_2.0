...
const template = require('../template/TnullPage'); // 生成空模板组件
module.exports = {
    create: (data) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                fs.writeFileSync(pagePath + '/Child' + data.formData.fileName + '.vue', template(data))// 生成页面
                resolve('生成空白页面')
            } catch (error) {
                console.error(error);
                reject(error)
...
/**操作按钮模板 */
const templateOptionBtn = (data, type) => `<el-button type="${data.formData.optionColor}" size="small" plain @click="$router.push('${data.pageInfo.routerPath}')">${data.formData.optionBtn}</el-button>`
